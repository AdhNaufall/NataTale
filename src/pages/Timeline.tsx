...
export default function Timeline({ memories, onEdit, onDelete }: { memories: any[], onEdit?: (m: any) => void, onDelete?: (id: string) => void }) {
  ...
  const scrollToMonth = (monthKey: string) => {
    ...
    const element = document.getElementById(`month-section-${monthKey}`);
    if (element) {
    ...
    <div className="relative min-h-screen pt-12 pb-24 px-4 max-w-2xl md:max-w-6xl mx-auto">
      ...
      <div className="fixed top-6 right-6 z-40">
        <button 
          ...
          aria-label="Lompat ke bulan memori"
          className="w-10 h-10 bg-white shadow-lg rounded-full flex items-center justify-center text-lavender hover:scale-110 transition-transform border border-softblue/20 cursor-pointer"
        >
        ...
        <AnimatePresence>
          {showMonthPicker && (
            <motion.div
              ...
              className="absolute right-0 mt-3 w-56 bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/80 p-3 z-50 space-y-1.5"
            >
              <div className="text-[10px] font-bold uppercase tracking-widest text-slate/50 px-2 py-1 border-b border-gray-100">
                Jump to Month
              </div>
              <div className="max-h-60 overflow-y-auto space-y-1 pr-1">
                {Object.entries(groupedMemories).map(([monthYear, monthMemories]) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs font-semibold text-slate hover:bg-softblue/15 hover:text-blue-600 rounded-xl transition-colors flex justify-between items-center cursor-pointer"
                  >
                    <span>{monthYear}</span>
                    <span className="text-[10px] font-bold bg-softblue/20 text-blue-600 px-2 py-0.5 rounded-full">
      ...
      <div className="space-y-16">
        {Object.entries(groupedMemories).map(([monthYear, monthMemories]) => (
          <div key={monthYear} id={`month-section-${monthYear}`} className="relative">
            {/* Sticky Month Header */}
                    ...
                    >
                      ...
                      <div className="px-1 md:px-2">
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-2 mt-3">
                          ...
                          {memory.rating && (
                            <span className="px-2.5 py-1 bg-amber-50 text-amber-600 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 border border-amber-200/60 shadow-sm">
                              <Star className="w-3 h-3 fill-amber-400 text-amber-400" /> {memory.rating}/5
                            </span>
                          )}
                        </div>
